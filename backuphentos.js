...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Phentos ❤ BACKUP" }, status: "online" });
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Phentos (Koruma)" }).catch();
};
...
client.on("channelCreate", async channel => {
  ...
  channel.delete({reason: "Phentos (Kanal Koruma)"});
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "Phentos (Kanal Koruma)" }).then(async kanal => {
...
client.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Phentos (Rol Koruma)" });
  cezalandir(entry.executor.id, "jail");
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
...
client.on("roleDelete", async role => {
  ...
  let yeniRol = await role.guild.roles.create({
    ...
    reason: "Phentos (Rol silindiği için tekrar oluşturuldu!)"
  });
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    console.log(`Rol veri tabanı düzenlendi! Phentos Selam Söylüyor!`);
  };
};